...
# 풀이 전략
선입 선출이라 Que 를 쓰기로 했고 답안을 계속 기록하기위해 set 을 썼다.
그리고 무엇보다 외부에서 오는 값은 모두 검사되어야한다는 규칙을 지키기 위해 vali check 을 했다.
...
class Solution {
    public int[] solution(int n, String[] words) {
        ...
        if(words[0].length() < 2 && words[0].length() > 50) return new int[]{1,1};
        participantMap.put(1,2);
        int person = que.poll();
        que.add(person);
        answerSet.add(words[0]);
        for(int i = 1; i < words.length; i++){
            person = que.poll();
            if(words[i].length() < 2 && words[i].length() > 50) return new int[]{person, participantMap.get(person)};
            if(answerSet.contains(words[i]) || words[i-1].charAt(words[i-1].length()-1) != words[i].charAt(0)){
                return new int[]{person, participantMap.get(person)};
            }
